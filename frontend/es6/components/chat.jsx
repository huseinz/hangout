...
class Chat extends React.Component{
    ...
    sendMessage(){
        if(this.state.str_message.length === 0)
            return;
    ...
    render(){
        return(
            <div>
                ...
                <input className="warning" type="hidden" value={this.state.str_message} onChange={this.handleInputChange} />
                <form className="form">
                    <fieldset className="form-group form-textarea">
                        <label>MESSAGE:</label>
                        <textarea className="form-control" value={this.state.str_message} onChange={this.handleInputChange}></textarea>
                    </fieldset>
                </form>
                <button className="btn btn-default btn-ghost" id="sendbtn" onClick={this.sendMessage}>SEND</button>
            </div>
        )
    };
}
